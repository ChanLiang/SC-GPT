...
MODEL_CLASSES = {
    ...
    # 'dialogpt':(AutoTokenizer, AutoModelForCausalLM)
}


class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train', block_size=512, max_seq=256):
...
class TextSeqDataset(Dataset): # 当做纯文本处理了，没有用到 &
    def __init__(self, tokenizer, args, file_path='train', block_size=512, max_seq=256, seperator=' & '):
        assert os.path.isfile(file_path)
        directory, filename = os.path.split(file_path)
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            with open(file_path, encoding="utf-8") as f:
                for line in f:
                    ...
                    code_str = code_str.strip() # persona + context
                    if len(raw_str.split()) > max_seq -1: # 把response截掉了怎么办...
                    ...
                    if len(tokenized_text) < max_seq: # 统一padding到max_seq
                    ...
                    self.examples.append(tokenized_text)
                    self.masks.append(mask) # 真实长度
                    self.labels.append(label) # 和example一样？？都没有错位
...
def train(args, train_dataset, model, tokenizer):
    ...
    for e in train_iterator:
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            #     print ('inputs.shape, masks.shape, labels.shape = ', inputs.shape, masks.shape, labels.shape) # torch.Size([4, 512]) torch.Size([4, 512])
            ...
            #     print (tokenizer.decode(inputs[0].tolist(), skip_special_tokens=True))
            #     print (tokenizer.decode(labels[0].tolist(), skip_special_tokens=True))
            #     print ()
            ...
            # print ('loss = ', loss) # tensor(0.9079, device='cuda:3', grad_fn=<NllLossBackward>)
...
def main():
    ...
    # Training
    if args.do_train:
        if args.local_rank not in [-1, 0]: # multi-GPU
